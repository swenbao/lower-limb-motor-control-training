/**
 * 網站內容資料
 * FOREWORD：課程緣起，首頁預設收合、展開後顯示的前言
 * CATEGORIES：四個訓練分類的文字內容，依訓練邏輯排序（由淺入深）
 * SCHEDULE：12 堂課表資料，依「週」分組，每週包含 2 堂課，每堂課包含放鬆軟組織時間與該堂的動作清單
 */
const FOREWORD =
  "青春期是骨骼發展特別重要的時期，18 歲時可達到骨量高峰的 90%。兒童及青少年運動員會因正值骨骼發育期而常有骨骺過度使用損傷，以及骨骼應力損傷。青少年運動傷害盛行率的數據顯示，在運動比賽中造成的非接觸性運動傷害約有 18%；而在訓練和練習中造成的非接觸性運動傷害則約 37%。非接觸性傷害常見因子包括足踝支撐力不佳導致不正確的身體代償、僵硬的落地動作、錯誤的動作模式造成動態膝外翻（valgus）力矩過大、不夠了解身體肌群的使用導致肌群不協調╱本體感覺不佳等等，上述因子多可透過即早訓練調整達到改善，因此本團隊針對兒童運動員設計一套關於下肢動作控制訓練的課程，透過正確的放鬆與啟動使運動員能更加瞭解身體肌肉的使用，並學會各關節之間正確的身體排列，希望能減少非接觸傷害的風險因子，以達降低運動傷害。";
